...
const CONTRACT_PACKAGE_NAME: &str = "counter_package_hash";
...
const ARG_AMOUNT_TO_ADD: &str = "add_amount";
...
// Entry point that increments the count value by value provided by the caller.
#[no_mangle]
pub extern "C" fn counter_inc() {
    let how_much: i32 = runtime::get_named_arg(ARG_AMOUNT_TO_ADD);
    let uref: URef = runtime::get_key(COUNT_KEY)
        .unwrap_or_revert_with(ApiError::MissingKey)
        .into_uref()
        .unwrap_or_revert_with(ApiError::UnexpectedKeyVariant);
    storage::add(uref, how_much); 
}
